...
# 计算FPR和TPR
fpr, tpr, thresholds = roc_curve(ano_label, ano_score_final)

# 保存AUC、FPR和TPR到CSV文件
import csv
with open('draw_data/{}.csv'.format(args.dataset), 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['FPR', 'TPR'])  # 写入标题行
    writer.writerows(zip(fpr, tpr))  # 写入FPR和TPR
    writer.writerow(['AUC', auc])  # 写入AUC
